...
async function loadFromSearchParams() {
    ...
    let ps = []
    urlParams.getAll("track").forEach(async (track) => {
...
//        map.getSource(pair[0]).getData().then((d) => {
//            coords.push(d.features[0].geometry.coordinates)
//        })
    }

//    await Promise.all(Array.from( sources ).map(async ([k, v]) => {
//        let data_p = await map.getSource(k).getData();
//        promises.push(data_p);
//    })).then(() => {
...
//        console.log(values)
//         for (let feature of values) {
//            coords.push(feature[0].geometry.coordinates)
//        };
//        console.log(coords)
        //            .then(d => {
        //                const coordinates = d.features[0].geometry.coordinates;
        //                coords.concat(coordinates);
        //            });
...
//    Promise.resolve(sources.forEach(async (k, v) => {
//            await map.getSource(k).getData().then(d => {
//                const coordinates = d.features[0].geometry.coordinates;
//                coords.concat(coordinates);
//            });
//        })).then(() => {
