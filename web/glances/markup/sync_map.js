...
function init() {
    ...
    loadSearchParams().then(() => console.log(sources));
}
function loadSearchParams() {
    const urlParams = new URLSearchParams(window.location.search);
    urlParams.getAll("track").forEach(track => {
        trackFromUrl(track, track);
    });
}
...
function selectTrack() {
    ...
    trackFromUrl(target_url, target_url)
}
function trackFromUrl(url, sourceName) {
    fetch(url).then(r => {
        ...
        addSourceLayer(sourceName, blobUrl)
        zoomToSource(sourceName)
        document.getElementById("inputTrackColor").value = randomPastelColors()
    })
}

function addSourceLayer(sourceName, blobUrl) {
...
function zoomToSources() {
    ...
    for (let pair of sources) {
        promises.push(map.getSource(pair[0]).getData().then(d => coords.push(d)));
    }
